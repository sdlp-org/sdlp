...
describe('DID Resolver', () => {
  describe('resolveDid', () => {
    ...
    it('should handle did:key without z prefix', async () => {
      const invalidDid =
        'did:key:MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK';
      const result = await resolveDid(invalidDid);
    ...
    it('should handle did:key with wrong multicodec', async () => {
      ...
      const invalidDid =
        'did:key:z4MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK';
      const result = await resolveDid(invalidDid);
    ...
    it('should handle test domains gracefully', async () => {
      const testDids = ['did:web:acme.example', 'did:web:test.example'];
    ...
    it('should handle malformed DIDs', async () => {
      const malformedDids = ['not-a-did', 'did:', 'did:invalid', ''];

...
import {
  generateDidKey,
  generateDidKeyFromBase64url,
} from '../scripts/generate-did-key.js';
...
describe('Type Guards', () => {
  describe('isValidDIDIdentifier', () => {
    it('should validate valid DID identifiers', () => {
      expect(
        isValidDIDIdentifier(
          'did:key:z2DUDndJSiGoP1cZLx6tEeFr9GugkN4QqbFNcbGKUyR8p9g'
        )
      ).toBe(true);
  ...
  describe('isValidSDLPLink', () => {
    it('should validate valid SDLP links', () => {
      expect(
        isValidSDLPLink(
          'sdlp://eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0'
        )
      ).toBe(true);

...
function validateKeyPair(
  privateKeyJwk: TestKeyFixtures['ed25519_private_key_jwk'],
  publicKeyJwk: TestKeyFixtures['ed25519_public_key_jwk']
): ValidationResult {
  ...
  try {
    ...
    if (privateKeyJwk.x !== publicKeyJwk.x) {
      result.errors.push(
        'Public key components (x) do not match between private and public JWKs'
      );
      result.valid = false;
    }
    ...
    const derivedPublicKeyDer = publicKey.export({
      format: 'der',
      type: 'spki',
    });
    ...
    result.errors.push(
      `Cryptographic validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    result.valid = false;
  }
...
function validateDidKey(
  ...
  try {
    ...
    if (decoded.length < 33 || decoded[0] !== 0xed) {
      result.errors.push(
        'DID key must be Ed25519 with correct multicodec prefix (0xed)'
      );
      result.valid = false;
      return result;
    }
    ...
    result.errors.push(
      `DID key validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    result.valid = false;
  }
...
function validateTestFixtures(): ValidationResult {
  ...
  try {
    ...
    return {
      valid: false,
      errors: [
        `Failed to parse test fixtures: ${error instanceof Error ? error.message : 'Unknown error'}`,
      ],
      warnings: [],
    };
  }
  ...
  // Validate key pair
  const keyPairResult = validateKeyPair(
    fixtures.ed25519_private_key_jwk,
    fixtures.ed25519_public_key_jwk
  );
  result.errors.push(...keyPairResult.errors);
  result.warnings.push(...keyPairResult.warnings);
  if (!keyPairResult.valid) {
    result.valid = false;
  }

  // Validate DID key
  const didKeyResult = validateDidKey(
    fixtures.did_key_identifier,
    fixtures.ed25519_public_key_jwk
  );
  result.errors.push(...didKeyResult.errors);
  result.warnings.push(...didKeyResult.warnings);
  if (!didKeyResult.valid) {
    result.valid = false;
  }

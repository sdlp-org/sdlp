...
function parseSDLPLink(link: string): ParsedSDLPLink | null {
  try {
    ...
    const jws = JSON.parse(new TextDecoder().decode(jwsJson)) as JWSStructure;

    // Decode protected header
    const protectedHeaderJson = base64urlDecode(jws.protected);
    const protectedHeader = JSON.parse(
      new TextDecoder().decode(protectedHeaderJson)
    ) as ProtectedHeader;

    // Decode metadata payload
    const metadataJson = base64urlDecode(jws.payload);
    const metadata = JSON.parse(
      new TextDecoder().decode(metadataJson)
    ) as Metadata;
...
function validateTestVector(
  ...
  if (
    !parsed.jws?.protected ||
    !parsed.jws?.payload ||
    !parsed.jws?.signature
  ) {
    result.errors.push(
      `${prefix}: JWS must have protected, payload, and signature`
    );
    result.valid = false;
  }
  ...
  if (parsed.protectedHeader) {
    if (parsed.protectedHeader.alg !== 'EdDSA') {
      result.errors.push(
        `${prefix}: Algorithm must be 'EdDSA', got '${parsed.protectedHeader.alg}'`
      );
      result.valid = false;
    }

    if (!parsed.protectedHeader.kid?.startsWith('did:')) {
  ...
  if (parsed.metadata) {
    if (parsed.metadata.v !== 'SDL-1.0') {
      result.errors.push(
        `${prefix}: Version must be 'SDL-1.0', got '${parsed.metadata.v}'`
      );
      result.valid = false;
    }

    if (!parsed.metadata.sid?.startsWith('did:')) {
    ...
    if (!['none', 'br', 'gz', 'zstd'].includes(parsed.metadata.comp)) {
      result.errors.push(
        `${prefix}: comp must be 'none', 'br', 'gz', or 'zstd'`
      );
      result.valid = false;
    }
  ...
  if (vector.expected.valid) {
    ...
    if (
      vector.expected.payload === null ||
      vector.expected.payload === undefined
    ) {
      result.warnings.push(
        `${prefix}: Valid vector should specify expected payload`
      );
    }

    if (!vector.expected.payloadType) {
      result.warnings.push(
        `${prefix}: Valid vector should specify expected payloadType`
      );
    }

    if (!vector.expected.senderDid) {
      result.warnings.push(
        `${prefix}: Valid vector should specify expected senderDid`
      );
    }
    ...
    if (
      parsed.metadata?.sid &&
      vector.expected.senderDid !== parsed.metadata?.sid
    ) {
      result.errors.push(
        `${prefix}: expected.senderDid doesn't match metadata.sid`
      );
      result.valid = false;
    }
    ...
    if (
      ...
      vector.expected.payloadType !== parsed.metadata?.type
    ) {
      result.errors.push(
        `${prefix}: expected.payloadType doesn't match metadata.type`
      );
      result.valid = false;
    }
  } else {
    ...
    if (!vector.expected.error) {
      result.errors.push(
        `${prefix}: Invalid vector must specify expected error`
      );
      result.valid = false;
    }
    ...
    if (vector.expected.error && !validErrors.includes(vector.expected.error)) {
      result.errors.push(
        `${prefix}: error must be one of: ${validErrors.join(', ')}`
      );
      result.valid = false;
    }
    ...
    if (vector.expected.payload !== null) {
      result.warnings.push(
        `${prefix}: Invalid vector should have payload: null`
      );
    }

    if (vector.expected.payloadType !== null) {
      result.warnings.push(
        `${prefix}: Invalid vector should have payloadType: null`
      );
    }

    if (vector.expected.senderDid !== null) {
      result.warnings.push(
        `${prefix}: Invalid vector should have senderDid: null`
      );
    }
  }
...
function validateTestVectors(filePath?: string): ValidationResult {
  const testVectorsPath =
    filePath ?? path.join(__dirname, '..', 'mvp-test-vectors.json');
  ...
  try {
    ...
    return {
      valid: false,
      errors: [
        `Failed to parse test vectors: ${error instanceof Error ? error.message : 'Unknown error'}`,
      ],
      warnings: [],
    };
  }
...
function main(): void {
  ...
  for (let index = 0; index < args.length; index++) {
    const currentArgument = args.at(index);
    const nextArgument = args.at(index + 1);

    if (currentArgument === '--file' && nextArgument) {
      ...
      console.log('Options:');
      console.log(
        '  --file <path>  Path to test vectors file (default: ../mvp-test-vectors.json)'
      );

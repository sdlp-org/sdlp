...
export function parseSDLPLink(link: string): ParsedSDLPLink {
  try {
    ...
    const jwsObject = JSON.parse(jwsDecoded.toString()) as JWSCompactSerialization
    ...
    return {
      ...
      metadata: {
        ...
        expiration: coreMetadata.exp ? new Date(coreMetadata.exp * 1000).toISOString() : null,
        notBefore: coreMetadata.nbf ? new Date(coreMetadata.nbf * 1000).toISOString() : null,
      },
    }
  } catch (error) {
    return {
...
export function formatParsedLink(parsed: ParsedSDLPLink): void {
  ...
  console.log(`  Payload Part: ${parsed.structure.payloadPart.length} characters`)
  console.log(
  ...
  // Warnings and notes
  console.log('⚠️  Notes:')
  console.log('  - This parser only shows structure, it does NOT verify signatures')
  console.log('  - Use a proper SDLP verifier to validate authenticity and integrity')
  if (parsed.metadata.expiration) {
    const now = new Date()
